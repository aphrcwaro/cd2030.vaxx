...
import { Schemas, VAXX_AUTHORITY } from '../../base/common/network.js';
import { join, posix } from '../../base/common/path.js';
import { IProcessEnvironment, isLinuxSnap, isMacintosh, isWindows, OS } from '../../base/common/platform.js';
...
export class CodeApplication extends Disposable {
	...
	private static readonly SECURITY_PROTOCOL_HANDLING_CONFIRMATION_SETTING_KEY = {
		...
		[Schemas.vaxxRemote]: 'security.promptForRemoteFileProtocolHandling' as const
	};
	...
	private configureSession(): void {
		...
		const isUrlFromWindow = (requestingUrl?: string | undefined) => requestingUrl?.startsWith(`${Schemas.vaxxFileResource}://${VAXX_AUTHORITY}`);
		const isUrlFromWebview = (requestingUrl: string | undefined) => requestingUrl?.startsWith(`${Schemas.vaxxWebview}://`);
		...
		const supportedSvgSchemes = new Set([Schemas.file, Schemas.vaxxFileResource, Schemas.vaxxRemoteResource, Schemas.vaxxManagedRemoteResource, 'devtools']);
		...
		const isSafeFrame = (requestFrame: WebFrameMain | null | undefined): boolean => {
			for (let frame: WebFrameMain | null | undefined = requestFrame; frame; frame = frame.parent) {
				if (frame.url.startsWith(`${Schemas.vaxxWebview}://`)) {
					return true;
				}
			}
			return false;
		...
		session.defaultSession.webRequest.onBeforeRequest((details, callback) => {
			...
			if (uri.scheme === Schemas.vaxxWebview) {
				if (!isAllowedWebviewRequest(uri, details)) {
					this.logService.error('Blocked vaxx-webview request', details.url);
			...
			if (uri.scheme === Schemas.vaxxFileResource) {
				if (!isAllowedVsCodeFileRequest(details)) {
					this.logService.error('Blocked vaxx-file request', details.url);
					return callback({ cancel: true });
				}
			}
		...
		session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
			...
			if (contentTypes && Array.isArray(contentTypes)) {
				...
				// http://127.0.0.1:<port>/vaxx-remote-resource?path=
				if (!uri.path.endsWith(Schemas.vaxxRemoteResource) && contentTypes.some(contentType => contentType.toLowerCase().includes('image/svg'))) {
					return callback({ cancel: !isSvgRequestFromSafeContext(details) });
				}
			}
	...
	private registerListeners(): void {
		...
		app.on('accessibility-support-changed', (event, accessibilitySupportEnabled) => {
			this.windowsMainService?.sendToAll('vaxx:accessibilitySupportChanged', accessibilitySupportEnabled);
		});
		...
		app.on('web-contents-created', (event, contents) => {
			...
			if (contents?.opener?.url.startsWith(`${Schemas.vaxxFileResource}://${VSCODE_AUTHORITY}/`)) {
		...
		validatedIpcMain.handle('vaxx:fetchShellEnv', event => {
		...
		validatedIpcMain.on('vaxx:toggleDevTools', event => event.sender.toggleDevTools());
		validatedIpcMain.on('vaxx:openDevTools', event => event.sender.openDevTools());

		validatedIpcMain.on('vaxx:reloadWindow', event => event.sender.reload());

		validatedIpcMain.handle('vaxx:notifyZoomLevel', async (event, zoomLevel: number | undefined) => {
	...
	async startup(): Promise<void> {
		...
		// Setup vaxx-remote-resource protocol handler
	...
	private setupManagedRemoteResourceUrlHandler(mainProcessElectronServer: ElectronIPCServer) {
		...
		protocol.registerBufferProtocol(Schemas.vaxxManagedRemoteResource, (request, callback) => {
	...
	private async shouldBlockOpenable(openable: IWindowOpenable, windowsMainService: IWindowsMainService, dialogMainService: IDialogMainService): Promise<boolean> {
		...
		if (openableUri.scheme !== Schemas.file && openableUri.scheme !== Schemas.vaxxRemote) {

			// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			//
			// NOTE: we currently only ask for confirmation for `file` and `vaxx-remote`
		...
		if (checkboxChecked) {
			...
			const request = { channel: 'vaxx:disablePromptForProtocolHandling', args: openableUri.scheme === Schemas.file ? 'local' : 'remote' };
	...
	private getWindowOpenableFromProtocolUrl(uri: URI): IWindowOpenable | undefined {
		...
		// Remote path
		else if (uri.authority === Schemas.vaxxRemote) {

			// Example conversion:
			// From: vaxx://vaxx-remote/wsl+ubuntu/mnt/c/GitDevelopment/monaco
			//   To: vaxx-remote://wsl+ubuntu/mnt/c/GitDevelopment/monaco
			...
			const remoteUri = URI.from({ scheme: Schemas.vaxxRemote, authority, path, query, fragment: uri.fragment });
	...
	private setupSharedProcess(machineId: string, sqmId: string, devDeviceId: string): { sharedProcessReady: Promise<MessagePortClient>; sharedProcessClient: Promise<MessagePortClient> } {
		...
		this._register(sharedProcess.onDidCrash(() => this.windowsMainService?.sendToFocused('vaxx:reportSharedProcessCrash')));
	...
	private afterWindowOpen(): void {
		...
		// Remote Authorities
		protocol.registerHttpProtocol(Schemas.vaxxRemoteResource, (request, callback) => {
			callback({
				url: request.url.replace(/^vaxx-remote-resource:/, 'http:'),
		...
		if (isMacintosh && app.runningUnderARM64Translation) {
			this.windowsMainService?.sendToFocused('vaxx:showTranslatedBuildWarning');
		}
	}
	...
	private async resolveShellEnvironment(args: NativeParsedArgs, env: IProcessEnvironment, notifyOnError: boolean): Promise<typeof process.env> {
		try {
			...
			if (notifyOnError) {
				this.windowsMainService?.sendToFocused('vaxx:showResolveShellEnvError', errorMessage);
			} else {
	...
	private async updateCrashReporterEnablement(): Promise<void> {
		...
		try {
			...
			this.windowsMainService?.sendToFocused('vaxx:showArgvParseWarning');
		}
	}
