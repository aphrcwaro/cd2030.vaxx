...
import { getMenuBarVisibility, IFolderToOpen, INativeWindowConfiguration, IWindowSettings, MenuBarVisibility, hasNativeTitlebar, useNativeFullScreen, useWindowControlsOverlay, DEFAULT_CUSTOM_TITLEBAR_HEIGHT, TitlebarStyle, MenuSettings } from '../../window/common/window.js';
...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	get profile(): IUserDataProfile | undefined {
		...
		return profile ?? this.userDataProfilesService.defaultProfile;
	}
		...
		switch (type) {
			...
			case WindowError.PROCESS_GONE:
				...
				if (type === WindowError.UNRESPONSIVE) {
					if (this._win?.webContents?.isDevToolsOpened()) {
				...
				// Process gone
					...
					const { response, checkboxChecked } = await this.dialogMainService.showMessageBox({
						...
						message,
						detail: localize('appGoneDetailEmptyWindow', "We are sorry for the inconvenience. You can open a new empty window to start again."),
						checkboxLabel: undefined
					}, this._win);
	...
	private async destroyWindow(reopen: boolean, skipRestoreEditors: boolean): Promise<void> {
		...
		try {
			...
			if (reopen && this._config) {
				...
				let uriToOpen: IFolderToOpen | undefined = undefined;
				let forceEmpty = true;
