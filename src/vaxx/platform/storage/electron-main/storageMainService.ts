...
import { ApplicationStorageMain, ProfileStorageMain, InMemoryStorageMain, IStorageMain, IStorageMainOptions, IStorageChangeEvent } from './storageMain.js';
...
export class StorageMainService extends Disposable implements IStorageMainService {
	...
	isUsed(path: string): boolean {
		...
		for (const storage of [this.applicationStorage, ...this.mapProfileToStorage.values()]) {
			if (!storage.path) {
				continue;
			}
