...
export class StorageDatabaseChannel extends Disposable implements IServerChannel {
	...
	async call(_: unknown, command: string, arg: IBaseSerializableStorageRequest): Promise<any> {
		...
		const storage = await this.withStorageInitialized(profile);
	...
	private async withStorageInitialized(profile: IUserDataProfile | undefined): Promise<IStorageMain> {
		...
		try {
			...
			this.logService.error(`StorageIPC#init: Unable to init ${profile ? 'profile' : 'application'} storage due to ${error}`);
		}
