...
let _platform: Platform = Platform.Windows;
if (_isMacintosh) {
...
export const setTimeout0 = (() => {
	if (setTimeout0IsFaster) {
		...
		$globalThis.addEventListener('message', (e: any) => {
			if (e.data && e.data.vaxxScheduleAsyncWork) {
				for (let i = 0, len = pending.length; i < len; i++) {
					const candidate = pending[i];
					if (candidate.id === e.data.vaxxScheduleAsyncWork) {
						pending.splice(i, 1);
						candidate.callback();
						return;
					}
		...
		return (callback: () => void) => {
			...
			$globalThis.postMessage({ vaxxScheduleAsyncWork: myId }, '*');
		};
	}
