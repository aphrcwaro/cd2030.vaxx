...
export async function resolveNLSConfiguration({ userLocale, osLocale, userDataPath, commit, nlsMetadataPath }: IResolveNLSConfigurationContext): Promise<INLSConfiguration> {
	...
	try {
		...
		const mainLanguagePackPath = languagePack?.translations?.['vaxx'];
		if (
			!languagePack ||
		...
		const result: INLSConfiguration = {
			...
			// NLS: below properties are a relic from old times only used by vaxx-nls and deprecated
			locale: userLocale,
 ...
 * core (`vaxx`) and all extensions it supports, it points to the related
 * translation files.
 *
