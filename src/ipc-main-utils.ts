...
const ALLOWED_CHANNEL_PREFIXES = ['cd2030:'];
const isAllowedChannel = (channel: string): boolean => ALLOWED_CHANNEL_PREFIXES.some(prefix => channel.startsWith(prefix));
...
class ValidatedIpcMain implements Event.NodeEventEmitter {
	...
	once(channel: string, listener: ipcMainListener): this {
		ipcMain.once(channel, (event: IpcMainEvent, ...args: any[]) => {
			if (this.validateEvent(channel, event)) {
				listener(event, ...args);
			}
		});
